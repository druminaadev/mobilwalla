...
interface FavoriteSalonScreenProps {
  onNext: () => void;
  onPrevious: () => void;
  onSkip: () => void;
}

export const FavoriteSalonScreen: React.FC<FavoriteSalonScreenProps> = ({
  onNext,
  onPrevious,
  onSkip,
}) => {
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.mockupContainer}>
        <Image
          source={require('../../../assets/onboarding/favorite-salon-screen.png')}
      ...
      <View style={styles.card}>
        <Text style={styles.heading}>
          <Text style={styles.dark}>{'Keep Your '}</Text>
          <Text style={styles.gold}>{'Favorite\nSalon'}</Text>
          <Text style={styles.dark}>{' One Tap Away'}</Text>
        </Text>
...
const styles = StyleSheet.create({
  ...
  mockupImage: { width: width * 0.85, height: height * 0.5 },
  card: {
    height: height * 0.42,
